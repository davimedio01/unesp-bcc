...
//Ordenação: Gnome Sort
void GnomeSort(int vet[], int final){
    int indice = 0;
    while(indice <= final){
        if(indice == 0)
            indice++;
        
        if(vet[indice] <= vet[indice-1]) //Mais velhos na frente na fila
            indice++;
        else{
            int aux = vet[indice];
...
int main(){
    ...
    //Ordenando preferencial
    GnomeSort(preferencial.idade, preferencial.fim);
