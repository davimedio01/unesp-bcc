...
//Rotina do Exercício
void CalculaMultiplicaX(def_arvore arvore, int *resultado)
{
     if (arvore != NULL)
     {
          if (arvore->esq != NULL && arvore->dir != NULL)
     ...
     if (arvore->esq != NULL)
     {
          CalculaMultiplicaX(arvore->esq, &(*resultado));
     }
     if (arvore->dir != NULL)
     {
          CalculaMultiplicaX(arvore->dir, &(*resultado));
     }
}
...
int main()
{
     ...
     //Calcular soma dos elementos (que possuem dois filhos)
     int resultado = 0;
     CalculaMultiplicaX(arvore, &resultado);
     printf("\nResultado da Rotina: %d\n\n", resultado);
