...
//Rotina que devolve a quantidade de nós que possuem ao menos um filho
void QtdNosFilhos(def_arvore arvore, int *qtd_nos)
{
     int i, aux = 0;
     for (i = 0; i < GRAU; i++)
     {
          ...
          {
               aux = 1;
               QtdNosFilhos(arvore->filhos[i], &(*qtd_nos));
          }
     }

     //Caso tenha pelo menos um filho
     if(aux)
     {
          (*qtd_nos)++;
     }
...
int main()
{
     ...
     int qtd_nos = 0;
     QtdNosFilhos(arvore, &qtd_nos);
     printf("Qtd de nos que possuem ao menos um filho: %d\n\n", qtd_nos);
